...
public class PlotRemove implements CommandExecutor {
	...
	List<UUID> owners1 = new ArrayList<UUID>(); 
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		owners1.clear();
		OPlayers.clear();
		for (UUID id : region.getOwners().getUniqueIds()){ owners1.add(id); }
 

		int i;
		if (owners1.size() != 0){
			for(i=0; i < owners1.size(); i++){
				OPlayers.add(Bukkit.getOfflinePlayer(owners1.get(i)).getPlayer() );
			}
		}
		
		if (!OPlayers.contains(player) && !me.onno204.config.PlotPerms.CheckPerm(sender.getName(), "PlotInfo.admin")){
			sender.sendMessage(ChatColor.RED + "JIJ bent geen eigenaar van het plot: " + ChatColor.YELLOW + region.getId());
			sender.sendMessage(ChatColor.RED + "Doe "  + ChatColor.DARK_RED + "/plotinfo" + ChatColor.RED + " om de eigenaar te zien");
			return true;
		}

		if (!OPlayers.contains(player) && me.onno204.config.PlotPerms.CheckPerm(sender.getName(), "PlotInfo.admin")){
			sender.sendMessage(ChatColor.GREEN + "Je gebruikt nu je bypass rechten om het plot: " + ChatColor.YELLOW + region.getId() + ChatColor.GREEN + " Aan te passen"); 
		}
		...
		if (members.contains(playertje)){
			members.removePlayer( playertje ); 
		}else{
			sender.sendMessage(ChatColor.RED + "De speler was niet op het plot!");
			return true;
		}
		...
		sender.sendMessage(ChatColor.AQUA + "Plot: " + ChatColor.YELLOW + region.getId() + ChatColor.AQUA + "\n Player: " + ChatColor.YELLOW + Bukkit.getOfflinePlayer(args[0]).getName() + ChatColor.AQUA + " is met succes verwijderd. \n De mensen die nu toegang hebben zijn: " + ChatColor.YELLOW + members1.toString() );
